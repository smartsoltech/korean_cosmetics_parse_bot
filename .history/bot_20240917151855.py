...
from telegram.constants import Document
from dotenv import load_dotenv
from parser import Parser  # Импортируем Parser из модуля parser
...
async def handle_order(update: Update, context: CallbackContext) -> None:
    """Обработка данных заказа: получение ссылки, количества и опций"""
    global WAITING_FOR_ORDER, CURRENT_ORDER, LOADED_FILE
    message_text = update.message.text.lower()
...
def main() -> None:
    ...
    application.add_handler(MessageHandler(filters.Document.MIMEType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"), handle_uploaded_file))
