...
def run_watchdog():
    path_to_watch = "/usr/src/app/watched_directory"  # Укажите путь к директории, за которой будет следить watchdog
    file_watcher.start_watchdog(path_to_watch)
...
product_action_keyboard = ReplyKeyboardMarkup(
    [['Проверить товар', 'Оформление заказа']],
...
async def handle_order(update: Update, context: CallbackContext) -> None:
    ...
    if 'url' not in CURRENT_ORDER:
        ...
        if urls:
            ...
            # После получения ссылки выводим клавиатуру с вариантами
            await update.message.reply_text(f"Ссылка на товар получена: {clean_product_url}\nВыберите действие:", reply_markup=product_action_keyboard)
...
async def check_product(update: Update, context: CallbackContext) -> None:
    """Обработка команды 'Проверить товар' - парсинг и вывод информации"""
    global CURRENT_ORDER
    clean_product_url = CURRENT_ORDER.get('url')
    ...
    if clean_product_url:
        ...
        await update.message.reply_text("Сначала отправьте ссылку на товар.")

async def start_order_process(update: Update, context: CallbackContext) -> None:
    """Начало оформления заказа после получения ссылки"""
    global WAITING_FOR_ORDER, CURRENT_ORDER

    if 'url' in CURRENT_ORDER:
        WAITING_FOR_ORDER = True  # Активируем режим получения данных для заказа
        await update.message.reply_text(
            f"Ссылка на товар: {CURRENT_ORDER['url']}\nВведите количество товара.",
            reply_markup=order_keyboard
        )
    else:
        await update.message.reply_text("Пожалуйста, отправьте ссылку на товар для оформления заказа.")
...
def main() -> None:
    ...
    watchdog_thread.daemon = True  # Поток завершится, если завершится основной поток
    ...
    application.add_handler(CommandHandler("заказ", start_order))
    
    # Обработка команды 'цена [URL]'
    application.add_handler(MessageHandler(filters.Regex(r'^цена\s+https?://'), handle_price))

    # Обработка клавиш "Проверить товар" и "Оформление заказа"
    application.add_handler(MessageHandler(filters.Regex('Проверить товар'), check_product))
    application.add_handler(MessageHandler(filters.Regex('Оформление заказа'), start_order_process))

    # Обработка текста для оформления заказа
